...
//            for (int i = 0; i < queryL.size(); i++) {
    ...
    public ArrayList<PresentacionProducto> listarTodoPresentacionProductos() {
        ArrayList<PresentacionProducto> lista = new ArrayList<PresentacionProducto>();

        try {
            conect = con.conectar();
            conect.setAutoCommit(false);
            CallableStatement prcProcedimientoAlmacenado = conect.prepareCall(
                    "{ call listarComboPresentaciones }");
            //   prcProcedimientoAlmacenado.setInt(1, op);
            prcProcedimientoAlmacenado.execute();
            rs = prcProcedimientoAlmacenado.getResultSet();
            while (rs.next()) {
                PresentacionProducto obj = EntidadesMappers.getPresentacionProductoFromResultSet(rs);
                lista.add(obj);
            }
...
//            }
    ...
    public String modificarProductos(Productos obj) {
        ...
        try {
            ...
            prodProAlm.setLong(6, obj.getId_presentacion());
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////    
    ...
    //tipo Envase
    public ArrayList<PresentacionProducto> MostrarPresentacion() {
        ArrayList<PresentacionProducto> tp = new ArrayList<>();
        ...
        try {
            ...
            rs = prcProAl.getResultSet();
            while (rs.next()) {
                PresentacionProducto tpp = new PresentacionProducto(rs.getLong("idPresentaciones"),
                        rs.getString("nombre"));
                tp.add(tpp);
            }
            rs.close();
    ...
    public void InsertarPresentacion(String valor,String usuario) {
        String msg;
        try {
            conect = con.conectar();
            conect.setAutoCommit(false);
            CallableStatement cs = conect.prepareCall(
                    "{ call Insertar_Presentaciones(?,?,?) }");
            cs.setString(1, valor);
